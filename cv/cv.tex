...
% For submitted items inside lists, use a boolean toggle instead of comment environment
\newif\ifshowsubmitted
\showsubmittedtrue  % set to \showsubmittedfalse to hide submitted publications
...
\DeclareSourcemap{
  \maps[datatype=bibtex]{
    \map{
      \step[fieldsource=entrykey, match=becker2024bridging, final]
      \step[fieldset=addendum, fieldvalue={Submitted as full paper; accepted for presentation as extended abstract (top 40\% of full paper submissions)}]
    }
    \map{
      \step[fieldsource=entrykey, match=dagan2025resolving, final]
      \step[fieldset=addendum, fieldvalue={Submitted as full paper; accepted for presentation as extended abstract (top 40\% of full paper submissions)}]
    }
  }
}
...
\mycventry{January 2020 -- Present}{Assistant Professor}{University of Colorado, Boulder, CO}{Ann and H.\ J.\ Smead Aerospace Engineering Sciences Department \\ Computer Science Department, by courtesy}
...
\mycvitem{2025}{Smead Aerospace Engineering Department Outstanding Junior Faculty Award}
...
% Publication macros - optional argument for frontnote (e.g., "Under Review")
% Usage: \journal[Under Review]{citekey}
\newcounter{jnum}
\renewcommand{\thejnum}{J\arabic{jnum}}
\newcommand{\journal}[2][]{\refstepcounter{jnum}\item[{[\thejnum]}] \ifx\relax#1\relax\else\textbf{[#1]} \fi\fullcite{#2}\par\label{#2}}
\newcounter{jcnum}
\renewcommand{\thejcnum}{JC\arabic{jcnum}}
\newcommand{\jc}[2][]{\refstepcounter{jcnum}\item[{[\thejcnum]}] \ifx\relax#1\relax\else\textbf{[#1]} \fi\fullcite{#2}\par\label{#2}}
\newcounter{cnum}
\renewcommand{\thecnum}{C\arabic{cnum}}
\newcommand{\conf}[2][]{\refstepcounter{cnum}\item[{[\thecnum]}] \ifx\relax#1\relax\else\textbf{[#1]} \fi\fullcite{#2}\par\label{#2}}
\newcounter{onum}
\renewcommand{\theonum}{A\arabic{onum}}
\newcommand{\opub}[2][]{\refstepcounter{onum}\item[{[\theonum]}] \ifx\relax#1\relax\else\textbf{[#1]} \fi\fullcite{#2}\par\label{#2}}
\newcounter{enum}
\renewcommand{\theenum}{E\arabic{enum}}
\newcommand{\epub}[2][]{\refstepcounter{enum}\item[{[\theenum]}] \ifx\relax#1\relax\else\textbf{[#1]} \fi\fullcite{#2}\par\label{#2}}
\newcounter{pnum}
...
\ifshowsubmitted
\journal[Under Review]{dagan2026sensortasking}
\journal[Under Review]{monell2026vtwin}
\journal[In Revision]{kraske2026robustness}
\journal[In Revision]{boyer2026explanation}
\fi
...
\begin{itemize}[labelwidth=1cm,leftmargin=1cm,itemsep=0.1em]
\ifshowsubmitted
    \jc[Under Review]{ventura2026constraint}
    \jc[Under Review]{gupta2026edgebundles}
\fi
    \jc[Accepted, Awaiting Publication]{krusniak2026decisions}
    \jc[Accepted, Awaiting Publication]{laouar2026voi}
...
\ifshowsubmitted
\conf[Under Review]{gupta2026interception}
\fi
\conf{lee2025raoblackwellized}
\conf{moncton2024autonomous}
...
\begin{itemize}[labelwidth=1cm,leftmargin=1cm,itemsep=0.1em]
\opub{becker2025custody}
\opub{dagan2025hypothesis}
...
% These two publications were submitted as full papers to the AAMAS conference and were accepted as "extended abstracts", which meant they were in the top 40\% of submitted papers, however this is not considered an archival publication.

\begin{itemize}[labelwidth=1cm,leftmargin=1cm,itemsep=0.1em]
\epub{becker2024bridging}
\epub{dagan2025resolving}
\end{itemize}
...
\mycventry{2025 -- present}{satisfaculty}{\normalfont{\url{https://www.satisfaculty.com}}}{
    Python package for optimizing teaching schedules using integer linear programming. Used to create Smead AES Fall 2026 schedule.
}

\mycventry{2020}{ProjectAssigner.jl}{\normalfont{\url{https://github.com/zsunberg/ProjectAssigner.jl}}}{
    Software for optimally assigning student teams to projects based on preferences, friend groups, and skills~\mycite{sunberg2021fair}. Used to assign senior project groups in the AES department from 2020 to 2024.
}
...
\mycventry{2023 -- 2026}{PI: Human Centered Autonomy for Dynamic sUAS Target Search Operations}{NSF Center for Autonomous Air Mobility and Sensing (CAAMS) Sub-award}{\$287,000 (total award) \textbar{} \$152,000 (my share, approx.); Co-I: Nisar Ahmed, CU Boulder}
...
\mycventry{2022-2024}{CU PI: SURP: Fast planning under uncertainty with operational and safety guarantees}{NASA Jet Propulsion Laboratory}{\$180,000 (total award) \textbar{} \$150,000 (my share), PI: Federico Rossi, JPL; Co-I: Morteza Lahijanian, CU Boulder}
...
\begin{itemize}[labelwidth=1cm,leftmargin=1cm,itemsep=0.1em]
    \invited{Technion -- Israel Institute of Technology \textit{(Online)}}{Breaking the curse of dimensionality in POMDPs and games with sampling-based online planning}{2025}
...
\mycventry{Fa 2022, Sp 2024, Sp 2025}{ASEN 3728/3128 Aircraft Dynamics}{University of Colorado Boulder, typical enrollment: 140}{
    ...
    Nominated by students for the 2025 John and Mercedes Peebles Innovation in Education Award
}

\mycventry{Sp 2020--Sp 2026}{ASEN 5264 Decision Making under Uncertainty}{University of Colorado Boulder, typical enrollment: 30-60}{
...
\mycventry{Fa 2025}{ASEN 6519: Optimization: Applications and Algorithms}{University of Colorado Boulder, typical enrollment: 10-15}{
    New advanced graduate course that surveys optimization techniques created by me. \\
    Open-source course materials: \url{https://github.com/zsunberg/CU-Optimization-Materials}
}

\mycventry{Fa 2021, Fa 2023, Fa 2025}{ASEN 6519 Advanced Survey of Sequential Decision Making}{University of Colorado Boulder, typical enrollment: 10-15}{
...
\mycventry{2024 -- 2025}{Ofer Dagan}{Founding Machine Learning Engineer, Eiro Inc.}{}

\subsection{Graduated Ph.D. Thesis Advisees}
\mycventry{2020 -- 2025}{Qi Heng Ho {\normalfont(Co-advised with Morteza Lahijanian)}}{Assistant Professor, Virginia Tech}{}
...
\mycvitem{Fall 2020 -- Present}{Tyler Becker {\normalfont(Comprehensive exam spring 2026)}}
\mycvitem{Fall 2020 -- Present}{Ben Kraske {\normalfont(Comprehensive exams spring 2026, Received NSF GRFP)}}
...
\mycvitem{Fall 2024 -- Present}{Lisa Ventura {\normalfont(Prelim exam fall 2025)}}
\mycvitem{Fall 2024 -- Present}{Mark Boyer {\normalfont(Prelim exam fall 2025)}}
\mycvitem{Spring 2026 -- Present}{Mel Krusniak}
...
\begin{multicols}{2}
    ...
    Mark Stephenson \\
    Michael Moncton \\
    Matt Budd (University of Oxford)
\end{multicols}
\end{minipage}\\
\\
...
\mycvitem{2024--present}{Co-Organizer of \emph{Faculty Sanity} University-Wide Happy Hour}
\mycvitem{2024--present}{Undergraduate Curriculum Committee}
\mycvitem{2023--present}{Onboarding mentor for new faculty member}
\mycvitem{2023--present}{Undergraduate Operations Committee}{\small Optimized undergraduate teaching assignments for course scheduling for the department.}\\
\mycvitem{2023--2025}{Autonomous Systems Faculty Search Committee}
...
\mycvitem{2021, 2022, 2025}{Speaker for Tuskegee Airmen outreach event at CU}
...
\mycvitem{2025 -- 2026}{Area chair for Robotics: Science and Systems (RSS) Conference}
...
American Geophysical Union (AGU) Books\\
...
IEEE Intl. Conf. on Intelligent Robots and Systems (IROS)\\
...
\mycvitem{2026}{Proposal Reviewer for Israel Science Foundation (ISF)}
\mycvitem{2023}{NSF Computer and Information Science and Engineering (CISE) Proposal Review Panelist}
\mycvitem{2023}{Army Research Office (ARO) Proposal Reviewer}

\section{Professional Affiliations}
\mycvitem{$\sim$2010-Present}{American Institute of Aeronautics and Astronautics (AIAA) \textit{Member}}
\mycvitem{$\sim$2015-Present}{Institute of Electrical and Electronics Engineers (IEEE) \textit{Member}}
